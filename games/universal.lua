run(function()
    ...
    for _, connection in pairs(getconnections(ReplicatedStorage.DefaultChatSystemChatEvents.OnNewMessage.OnClientEvent)) do
        ...
        if debug.getinfo(func).name == "OnNewMessage" then
            local upvalues = debug.getupvalues(func)
            for _, val in pairs(upvalues) do
                if typeof(val) == "table" and getmetatable(val) and getmetatable(val).GetChannel then
                    local chatMeta = getmetatable(val)
                    local oldGetChannel = chatMeta.GetChannel
                    chatMeta.GetChannel = function(self, name)
                        local chan = oldGetChannel(self, name)
                        if chan and chan.AddMessageToChannel then
                            local old = chan.AddMessageToChannel
                            chan.AddMessageToChannel = function(self2, message)
                                local speaker = Players:FindFirstChild(message.FromSpeaker or "")
                                local tag = speaker and getTag(speaker)
                                if tag then
                                    message.ExtraData = message.ExtraData or {}
                                    message.ExtraData.Tags = message.ExtraData.Tags or {}
                                    table.insert(message.ExtraData.Tags, {
                                        TagText = tag,
                                        TagColor = Color3.new(1, 1, 1)
                                    })
                                end
                                return old(self2, message)
                            end
                        end
                        return chan
                    end
    ...
    Function = function(callback)
        if callback then
            ...
            pcall(function()
                ...
                if clientstore and clientstore:getState().Game.customMatch then
                    spoofedHostId = clientstore:getState().Game.customMatch.hostUserId
                end
            end)
            if spoofedHostId then
                ...
                mt.__index = newcclosure(function(self, key)
                    if self == LocalPlayer and key == "UserId" then
                        return spoofedHostId
                    end
                    return oldIndex(self, key)
                end)
                warn("[HostSpoofer] Spoofed UserId as host:", spoofedHostId)
            else
                warn("[HostSpoofer] Host ID not found.")
            end
        end
    end,
